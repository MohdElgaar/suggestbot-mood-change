...
def pie_chart(df, selected = 'Pos'):
    ...
    pie = go.Pie(labels=label, values=value, name = "pie")
...
def pospie(n_clicks):
    if n_clicks % 2 == 0:
        return {'display': 'none'}, pie_chart(placeholder_data)
    else:
        return {'display': 'block'}, pie_chart(placeholder_data, 'Pos')
...
def negpie(n_clicks):
    if n_clicks % 2 == 0:
        return {'display': 'none'}, pie_chart(placeholder_data, 'Neg')
    else:
        return {'display': 'block'}, pie_chart(placeholder_data, 'Neg')
